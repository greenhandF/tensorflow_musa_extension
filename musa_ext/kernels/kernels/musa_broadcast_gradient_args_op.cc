  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (!bcast.IsValid()) {
      ctx->SetStatus(errors::InvalidArgument(
          "Incompatible shapes: [", s0.SummarizeValue(10), "] vs. [",
    ...
    // Since we enforce HostMemory in registration, these outputs will reside in host memory.
    
    const size_t len0 = bcast.grad_x_reduce_idx().size();
    const size_t len1 = bcast.grad_y_reduce_idx().size();

    OP_REQUIRES_OK(ctx, ctx->allocate_output(0, TensorShape({static_cast<int64_t>(len0)}), &r0));
    OP_REQUIRES_OK(ctx, ctx->allocate_output(1, TensorShape({static_cast<int64_t>(len1)}), &r1));
...
// Register the kernel.
// Important: BroadcastGradientArgs usually takes shape inputs (int32/int64) and produces 
// reduction indices (int32/int64). These are metadata operations. 
// Even if the Op is placed on MUSA, the inputs and outputs are shapes, so we strictly 
// use HostMemory to avoid unnecessary H2D and D2H copies for shape logic.
