...
class MusaBroadcastToOp : public MusaOpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    BCast bcast(BCast::FromShape(input_tensor.shape()), BCast::FromShape(output_shape),
                true);
    
    OP_REQUIRES(ctx, bcast.IsValid(),
                errors::InvalidArgument(
                    "Incompatible shapes: ", input_tensor.shape().DebugString(), " vs. ",
    ...
    OP_REQUIRES(ctx, BCast::ToShape(bcast.output_shape()) == output_shape,
                errors::InvalidArgument("Unable to broadcast tensor of shape ",
                                        input_tensor.shape(), " to tensor of shape ",
                                        output_shape));
    ...
    if (y_shape.size() > 8) {
        OP_REQUIRES(ctx, false, errors::Unimplemented(
            "BroadcastTo with effective rank > 8 is not supported by MUSA backend."));
    }

    auto& handle = GetHandleByCtx(ctx);

    Tensor zero_tensor;
    OP_REQUIRES_OK(ctx, ctx->allocate_temp(input_tensor.dtype(), output_shape, &zero_tensor));
    
    auto status_memset = Memset(handle, const_cast<char*>(zero_tensor.tensor_data().data()), zero_tensor.TotalBytes(), 0);
    ...
    OP_REQUIRES(ctx, mt_in.SetNdInfo(static_cast<int>(in_dims.size()), in_dims.data()) == mStatus::SUCCESS,
                errors::Internal("SetNdInfo for input failed"));
    OP_REQUIRES(ctx, mt_out.SetNdInfo(static_cast<int>(out_dims.size()), out_dims.data()) == mStatus::SUCCESS,
                errors::Internal("SetNdInfo for output failed"));
    OP_REQUIRES(ctx, mt_zero.SetNdInfo(static_cast<int>(out_dims.size()), out_dims.data()) == mStatus::SUCCESS,
