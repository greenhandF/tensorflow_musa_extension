...
struct DivNoNanStrides { int s0, s1, s2, s3; };
struct DivNoNanDims { int d0, d1, d2, d3; };
...
class MusaDivNoNanOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    auto calc_strides = [&](const TensorShape& shape, const TensorShape& out_s) -> DivNoNanStrides {
        int64_t raw_st[4];
        int64_t acc = 1;
        for (int i = 3; i >= 0; --i) {
            raw_st[i] = acc;
...
#define REGISTER_MUSA_DIV_NO_NAN(TYPE)                                  \
  REGISTER_KERNEL_BUILDER(Name("DivNoNan")                              \
                              .Device("MUSA")                           \
