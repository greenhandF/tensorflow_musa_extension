...
class MusaPowOp : public MusaOpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES(ctx, t0.SetNdInfo(static_cast<int>(v0.size()), v0.data()) == mStatus::SUCCESS, 
                errors::Internal("SetNdInfo t0 failed"));
    OP_REQUIRES(ctx, t1.SetNdInfo(static_cast<int>(v1.size()), v1.data()) == mStatus::SUCCESS, 
                errors::Internal("SetNdInfo t1 failed"));
    OP_REQUIRES(ctx, t_out.SetNdInfo(static_cast<int>(v_out.size()), v_out.data()) == mStatus::SUCCESS, 
    ...
    OP_REQUIRES(ctx, status == ::musa::dnn::Status::SUCCESS,
                errors::Internal("MUSA Pow execution failed. Status: ", static_cast<int>(status)));
  }
};

#define REGISTER_MUSA_POW(TYPE)                                          \
  REGISTER_KERNEL_BUILDER(Name("Pow").Device("MUSA").TypeConstraint<TYPE>("T"), MusaPowOp<TYPE>);
