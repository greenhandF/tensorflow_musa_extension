 ...
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if ((delta > 0 && start < limit) || (delta < 0 && start > limit)) {
        size = static_cast<int64>(std::ceil(std::abs((double)(limit - start) / (double)delta)));
    }
    ...
    auto status = musaMemcpy(device_ptr, host_data.data(), size * sizeof(T), musaMemcpyHostToDevice);
    
    OP_REQUIRES(ctx, status == musaSuccess,
                errors::Internal("MusaRangeOp: musaMemcpy failed. Error code: ", static_cast<int>(status)));
  }
};
