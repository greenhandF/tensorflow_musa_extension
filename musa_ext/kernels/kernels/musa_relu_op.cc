...
public:
    ...
    void Compute(OpKernelContext* ctx) override {
        ...
        OP_REQUIRES(ctx, status == ::musa::dnn::Status::SUCCESS,
                   errors::Internal("MUSA muDNN Unary ReLU execution failed. Status: ", (int)status));
    }
};

REGISTER_KERNEL_BUILDER(
    Name("Relu").Device("MUSA").TypeConstraint<float>("T"),
    MusaReluOp<float>);
...
REGISTER_KERNEL_BUILDER(
    Name("Relu").Device("MUSA").TypeConstraint<double>("T"),
    MusaReluOp<double>);
