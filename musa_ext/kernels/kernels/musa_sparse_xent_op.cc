...
class MusaSparseSoftMaxCrossEntroyWithLogitsOp : public MusaOpKernel {
 public:
  explicit MusaSparseSoftMaxCrossEntroyWithLogitsOp(OpKernelConstruction* context)
  ...
  void Compute(OpKernelContext* context) override {
    ...
    OP_REQUIRES(context, logits.dims() == 2, 
                errors::InvalidArgument("logits must be 2-D, but got shape ", logits.shape().DebugString()));
    OP_REQUIRES(context, labels.dims() == 1, 
                errors::InvalidArgument("labels must be 1-D, but got shape ", labels.shape().DebugString()));
    OP_REQUIRES(context, logits.dim_size(0) == labels.dim_size(0),
                errors::InvalidArgument("logits and labels must have the same batch dimension"));
    ...
    OP_REQUIRES_OK(context, context->allocate_output(0, labels.shape(), &loss_tensor));
    Tensor* back_tensor = nullptr;
    OP_REQUIRES_OK(context, context->allocate_output(1, logits.shape(), &back_tensor));
    ...
    OP_REQUIRES_OK(context, context->allocate_temp(logits.dtype(), logits.shape(), &log_probs_t));
    OP_REQUIRES_OK(context, context->allocate_temp(logits.dtype(), labels.shape(), &gathered_log_t));
    OP_REQUIRES_OK(context, context->allocate_temp(logits.dtype(), labels.shape(), &ones_t));
    ...
    // 【修正点】: 显式强转 static_cast<int> 解决歧义报错
    std::vector<int64_t> fake_2d_shape = {batch_size, 1};
    labels_m.SetNdInfo(static_cast<int>(fake_2d_shape.size()), fake_2d_shape.data());
    ones_m.SetNdInfo(static_cast<int>(fake_2d_shape.size()), fake_2d_shape.data());

    mScatter scatter_op;
    scatter_op.SetMode(mScatter::Mode::SUB); 
    scatter_op.Run(h, back_m, labels_m, ones_m, 1, nullptr);

    VLOG(1) << ">>> [MUSA_TRACE] SparseSoftmaxCrossEntropy computed successfully.";
  }
};
