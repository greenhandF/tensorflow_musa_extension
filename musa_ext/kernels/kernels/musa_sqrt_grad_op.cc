...
class MusaSqrtGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    BCast bcast(BCast::FromShape(y.shape()),
                BCast::FromShape(dy.shape()),
                false);
    ...
    auto status_y = musaMemcpy(h_y.data(), y.flat<T>().data(), y_bytes, musaMemcpyDeviceToHost);
    auto status_dy = musaMemcpy(h_dy.data(), dy.flat<T>().data(), dy_bytes, musaMemcpyDeviceToHost);
    ...
    if (y.shape() == dy.shape()) {
        ...
        auto pad_shape_to_nd = [](const TensorShape& s, int n_dims) -> TensorShape {
            TensorShape new_s = s;
        ...
        auto compute_strides_for_broadcast = [&](const TensorShape& shape, std::vector<int64_t>& strides, const TensorShape& result_s) {
    ...
    auto status_dx = musaMemcpy(dx->flat<T>().data(), h_dx.data(), dx_bytes, musaMemcpyHostToDevice);
