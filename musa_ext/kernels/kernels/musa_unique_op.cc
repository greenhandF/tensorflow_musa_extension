...
class MusaUniqueOp : public MusaOpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    Tensor tmp_counts;
    OP_REQUIRES_OK(ctx, ctx->allocate_temp(DataTypeToEnum<OutIdxT>::value, input.shape(), &tmp_counts));
    ...
    ::musa::dnn::MemoryMaintainer maintainer = musa_device->GetMemMaintainer(mem_alloc_func);
    ...
    OP_REQUIRES(ctx, status == mStatus::SUCCESS, errors::Internal("muDNN Unique SetMode failed"));

    status = op.Run(handle, t_temp_out, t_idx, t_counts, t_in, maintainer);

    OP_REQUIRES(ctx, status == mStatus::SUCCESS, 
                errors::Internal("MUSA muDNN Unique execution failed. Status: ", (int)status));

    std::vector<OutIdxT> host_counts(num_elements);
    musaMemcpy(host_counts.data(), tmp_counts.flat<OutIdxT>().data(), counts_bytes, musaMemcpyDeviceToHost);
    ...
    OP_REQUIRES_OK(ctx, ctx->allocate_output(0, TensorShape({unique_count}), &out_values));

    if (unique_count > 0) {
        size_t data_bytes = unique_count * sizeof(T);
        musaMemcpy(out_values->flat<T>().data(), temp_out_values.flat<T>().data(), data_bytes, musaMemcpyDeviceToDevice);
    }
  }
};
