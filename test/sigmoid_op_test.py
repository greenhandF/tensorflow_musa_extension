...
class SigmoidOpTest(MUSATestCase):
  ...
  def _test_sigmoid(self, input_data, dtype, rtol=1e-5, atol=1e-8):
    ...
    with tf.device('/CPU:0'):
      cpu_result = tf.nn.sigmoid(x)
    
    # Test on MUSA
    with tf.device('/device:MUSA:0'):
      musa_result = tf.nn.sigmoid(x)
  ...
  def testSigmoidBasic(self):
    """Basic sigmoid test with various input values."""
    test_data = [-5.0, -2.0, -1.0, 0.0, 1.0, 2.0, 5.0]
    for dtype in [tf.float32, tf.float16, tf.bfloat16]:
      rtol = 1e-2 if dtype in [tf.float16, tf.bfloat16] else 1e-5
      atol = 1e-2 if dtype in [tf.float16, tf.bfloat16] else 1e-8
      self._test_sigmoid(test_data, dtype, rtol=rtol, atol=atol)

  def testSigmoid2D(self):
    """2D tensor sigmoid test."""
  ...
  def testSigmoidLargeValues(self):
    """Sigmoid test with large positive/negative values."""
    test_data = [-100.0, -50.0, 50.0, 100.0]
    for dtype in [tf.float32]:
      self._test_sigmoid(test_data, dtype)

  def testSigmoidZeroInput(self):
    """Sigmoid test with zero input."""
  ...
  def testSigmoidRandomData(self):
    """Sigmoid test with random data."""
    np.random.seed(42)
    test_data = np.random.uniform(-10, 10, size=[100]).tolist()
    for dtype in [tf.float32]:
      self._test_sigmoid(test_data, dtype)
